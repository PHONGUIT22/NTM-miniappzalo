...
import { NewsDetail } from './DetailComponents'; // Đã sửa đường dẫn
import { openChat } from "zmp-sdk/apis";

const NewsPage = ({
  newsArticles, // This will be the filtered list from App.js
  loadingNews,
  handleOpenLink,
  onBack,
  currentView,
  ...
  useEffect(() => {
    console.log('NewsPage rendered. Current searchTerm:', searchTerm, 'News articles count:', newsArticles.length);
  }, [searchTerm, newsArticles]);

  if (currentView === 'newsDetail') {
    return <NewsDetail newsId={selectedArticleId} onBack={onBack} />;
  }
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col font-sans text-gray-800 pb-16">
      <header className="bg-white shadow-md p-4 sticky top-0 z-50 pt-8">
        <div className="container mx-auto flex flex-col items-center px-4">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mt-4">Tin tức</h1>
      ...
      <main className="flex-grow container mx-auto p-4 md:p-8">
        <div className="grid grid-cols-1 gap-4 mb-12">
          {loadingNews ? (
            <p className="col-span-full text-center text-gray-600">Đang tải tin tức...</p>
          ) : newsArticles.length > 0 ? (
            newsArticles.map(article => (
              <div key={article.id} className="bg-white rounded-lg shadow-md overflow-hidden flex items-center p-4 transform transition-transform hover:scale-105">
                <img
                  src={article.image}
                  alt={article.title}
                ...
                <div className="flex-grow">
                  <h3 className="text-lg font-semibold text-gray-900 mb-1 leading-tight">{article.title}</h3>
                  <p className="text-sm text-gray-500">{article.date}</p>
                  <button
                    onClick={() => { console.log('Read More clicked for article ID:', article.id); handleOpenLink(article.link, 'news', article.id); }}
            ...
            <p className="col-span-full text-center text-red-600">Không tìm thấy tin tức nào phù hợp.</p>
          )}
        </div>
      </main>
